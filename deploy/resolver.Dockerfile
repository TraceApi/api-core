...
# Build the binary
# -o /resolver-api : Output filename
# ./cmd/api-resolver : Input entry point
# CGO_ENABLED=0 : Static binary (no external C dependencies)
RUN CGO_ENABLED=0 GOOS=linux go build -o /resolver-api ./cmd/api-resolver
...
COPY --from=builder /resolver-api .
